...
parser.add_argument("--tmp_dir", help="tmp dir for processing the ZHMolLigGraph", type=str, default = None)
...
def generate_pose(data_dir, prediction_model, device):
	...
	model = torch.load(prediction_model, map_location=device).to(device)
...
def select_pose(data_dir, prediction_model, selection_model, output_poses, ligand_file, output_file, device, num_poses):
	...
	test_loader=DataLoader(test_dataset, batch_size=1)
	model = torch.load(selection_model, map_location=device).to(device)
	...
	print("Start Molecular Optimization")
	for idx in tqdm(top_indices, desc="Molecular Optimization"):
...
if __name__ == "__main__":
	...
	select_pose(data_dir, prediction_model, selection_model, output_poses, ligand_file, output_file, device, num_poses)
